...
active_tasks = {"view": set(), "materialized_view": set(), "materialize": set()}
...
query_stats = {
    "view": {
        ...
        "first_measurement_skipped": False,
        "current_stats": {
            "qps": 0.0,
            "latency": 0.0,
    ...
    "cpu": {
        "measurements": [],
        "timestamps": [],
        "current_stats": {"usage": 0.0, "last_updated": 0.0},
    },
}
...
traffic_enabled = {"view": True, "materialized_view": True, "materialize": True}
...
def calculate_qps(source: str) -> float:
    ...
    logger.debug(
        f"QPS calculation for {source}: {total_queries} queries in {time_span:.2f}s = {qps:.2f} QPS"
    )
    return qps
...
async def refresh_materialize_pool():
    ...
    for attempt in range(max_retries):
        try:
            ...
            if materialize_pool:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            "Timeout while closing old pool - continuing anyway"
                        )
                    except Exception as e:
            ...
            logger.error(
                f"Error refreshing Materialize pool (attempt {attempt + 1}/{max_retries}): {str(e)}"
            )
        ...
        try:
            async with postgres_pool.acquire() as conn:
                ...
                latest_heartbeat.update(
                    {"insert_time": insert_time, "id": result["id"], "ts": result["ts"]}
                )
...
async def refresh_materialized_view():
    ...
    try:
        async with postgres_pool.acquire() as conn:
            ...
            logger.debug(
                f"Materialized view refresh completed in {refresh_duration:.2f} seconds"
            )
            await conn.execute(
                """
        ...
        logger.error(
            f"Materialized view refresh timed out after {time.time() - start_time:.2f} seconds",
            exc_info=True,
        )
        raise
    except Exception as e:
...
async def auto_refresh_materialized_view():
    ...
    while True:
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        ...
                        break
                    logger.warning(
                        f"Refresh attempt {attempt + 1} timed out, retrying..."
                    )
                    await asyncio.sleep(1)
                except Exception as e:
            ...
            logger.debug(
                f"Refresh cycle complete. Waiting {wait_time:.2f}s until next cycle."
            )
...
async def add_to_cart():
    ...
    async def delete_item():
        try:
            ...
            logger.error(
                f"Error removing item from shopping cart: {str(e)}", exc_info=True
            )
            raise
...
async def measure_query_time(
    query: str, params: Tuple, is_materialize: bool, source: str
) -> Tuple[float, any]:
    start_time = time.time()
    try:
        if is_materialize:
        ...
        current_time = time.time()
        async with stats_lock:
            # Skip the first measurement to avoid cold startup issues
            if not stats["first_measurement_skipped"]:
                stats["first_measurement_skipped"] = True
                logger.debug(
                    f"Skipping first measurement for {stats_key} to avoid cold startup"
                )
                return duration, result
            ...
            if (
                result
                and "adjusted_price" in result
                and result["adjusted_price"] is not None
            ):
                try:
            ...
            async with materialize_pool.acquire() as mz_conn:
                ...
                if pg_heartbeat and mz_heartbeat:
                    ...
                    if pg_id > mz_id:
                        ...
                        lag += (
                            pg_heartbeat["current_ts"] - pg_heartbeat["ts"]
                        ).total_seconds()
            ...
            response.update(
                {
                    ...
                    f"{source}_end_to_end_latency": stats["end_to_end_latency"]
                    if is_fresh
                    else None,
                    f"{source}_price": stats["price"] if is_fresh else None,
                    f"{source}_qps": stats["qps"] if is_fresh else None,
                    f"{source}_stats": calculate_stats(query_stats[source]["latencies"])
                    if is_fresh
                    else None,
                    f"{source}_end_to_end_stats": calculate_stats(
                        query_stats[source]["end_to_end_latencies"]
                    )
                    if is_fresh
                    else None,
                }
            ...
            if source == "materialized_view":
                ...
                if refresh_durations and is_fresh:
                    refresh_stats = {
                        ...
                        "p99": sorted(refresh_durations)[
                            int(len(refresh_durations) * 0.99)
                        ]
                        if len(refresh_durations) >= 100
                ...
                response.update(
                    {
                        "materialized_view_freshness": float(mv_freshness["age"])
                        if mv_freshness and is_fresh
                        else None,
                        "materialized_view_refresh_duration": float(
                            mv_freshness["refresh_duration"]
                        )
                        if mv_freshness and is_fresh
                        else None,
...
async def toggle_isolation_level():
    ...
    async with materialize_pool.acquire() as conn:
        new_level = (
            "strict serializable"
            if current_isolation_level == "serializable"
            else "serializable"
        )
...
async def check_materialize_index_exists():
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(
                f"Connection lost during index check (attempt {attempt + 1}/{max_retries})"
            )
            if attempt < max_retries - 1:
                await asyncio.sleep(retry_delay)
                retry_delay *= 2
...
async def toggle_traffic(source: str) -> bool:
    ...
    logger.info(
        f"Traffic for {source} is now {'enabled' if traffic_enabled[source] else 'disabled'}"
    )
    return traffic_enabled[source]


async def execute_query(
    source_key: str, is_materialize: bool, query: str, product_id: int
):
    try:
        ...
        task = asyncio.create_task(
            measure_query_time(query, [product_id], is_materialize, source_display)
        )
        ...
        if (
            traffic_enabled[source_key]
            and len(active_tasks[source_key]) < max_concurrent[source_key]
        ):
            asyncio.create_task(
                execute_query(source_key, is_materialize, query, product_id)
            )
    except Exception as e:
    ...
    while True:
        try:
            ...
            if len(active_tasks["view"]) < max_concurrent["view"]:
                asyncio.create_task(
                    execute_query("view", False, QUERIES["view"], product_id)
                )
            if (
                len(active_tasks["materialized_view"])
                < max_concurrent["materialized_view"]
            ):
                asyncio.create_task(
                    execute_query(
                        "materialized_view",
            ...
            if len(active_tasks["materialize"]) < max_concurrent["materialize"]:
                asyncio.create_task(
                    execute_query(
                        "materialize", True, QUERIES["materialize"], product_id
                    )
                )
...
async def get_concurrency_limits():
    ...
    return {"view": 1, "materialized_view": 5, "materialize": 5 if has_index else 1}
                ...
                try:
                    ...
                    if mv_stats:
                        async with stats_lock:
                            ...
                            logger.debug(
                                f"Updated MV freshness: {age:.2f}s (duration: {duration:.2f}s)"
                            )
                except Exception as e:
...
async def collect_container_stats():
    ...
    while True:
        try:
            current_time = time.time()
            containers = {
                "postgres_stats": "postgres",
                "materialize_stats": "materialize",
            }
            for stats_key, container_name in containers.items():
                ...
                if returncode != 0:
                    logger.error(
                        f"Error getting Docker stats for {container_name}: {process.stderr.read()}"
                    )
                else:
                    stats_str = (
                    ...
                    if len(stats_str) == 2:
                        ...
                        logger.debug(
                            f"Raw stats from {container_name}: CPU={cpu_str}%, MEM={mem_str}%"
                        )
                        try:
                            ...
                            async with stats_lock:
                                ...
                                logger.debug(
                                    f"Updated {stats_key} stats: CPU={cpu_usage}%, MEM={mem_usage}%"
                                )
                        except ValueError as e:
                            logger.error(
                                f"Error converting stats for {container_name}: {str(e)}"
                            )
                    else:
    ...
    for container_type in ["postgres_stats", "materialize_stats"]:
        ...
        if stats["cpu_measurements"]:
            cpu_stats = {
                ...
                "average": sum(stats["cpu_measurements"])
                / len(stats["cpu_measurements"]),
                "p99": sorted(stats["cpu_measurements"])[
                    int(len(stats["cpu_measurements"]) * 0.99)
                ]
                if len(stats["cpu_measurements"]) >= 100
                else max(stats["cpu_measurements"]),
            }
        if stats["memory_measurements"]:
            memory_stats = {
                "max": max(stats["memory_measurements"]),
                "average": sum(stats["memory_measurements"])
                / len(stats["memory_measurements"]),
                "p99": sorted(stats["memory_measurements"])[
                    int(len(stats["memory_measurements"]) * 0.99)
                ]
                if len(stats["memory_measurements"]) >= 100
                else max(stats["memory_measurements"]),
            }
        response[container_type] = {
    ...
    def _cleanup(self):
        ...
        try:
            ...
            if loop.is_running():
                ...
                logger.info(
                    "Cleanup: event loop not running, running cleanup synchronously"
                )
        ...
        try:
            async with postgres_pool.acquire() as conn:
                ...
                for item in cart_items:
                    ...
                    logger.debug(
                        f"Updated inventory for product {item['product_id']}: {amount:+d} units"
                    )
        except Exception as e:
